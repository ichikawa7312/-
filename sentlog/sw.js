...
self.addEventListener('fetch',event=>{
  ...
  const isFreshCritical=
    ...
    u.pathname.endsWith('/sentlog/index.html') ||
    u.pathname.endsWith('/sentlog/cloud-sync.js') ||
    u.pathname.endsWith('/sentlog/cloud-hotfix.js') ||
    /\/sentlog\/part0[1-9]\.txt$/.test(u.pathname);
  if(isFreshCritical){
    event.respondWith((async()=>{
      try{
        const r=await fetch(event.request,{cache:'no-store'});
        if(r&&(r.ok||r.type==='opaque')){
          const c=await caches.open(CACHE);await c.put(event.request,r.clone());
        }
        return r;
      }catch(e){
        return (await caches.match(event.request)) || (event.request.mode==='navigate'?await caches.match('./index.html'):Promise.reject(e));
      }
    })());
    return;
  }
  event.respondWith(caches.match(event.request).then(cached=>cached||fetch(event.request).then(async r=>{
    if(r&&(r.ok||r.type==='opaque')){const c=await caches.open(CACHE);c.put(event.request,r.clone())}
    return r;
  }).catch(()=>{if(event.request.mode==='navigate')return caches.match('./index.html');throw new Error('offline')})));
});
