(function () {
  ...
  window.SENTLOG_BUILD = 'v1.20';
  const header = document.querySelector('header');
  if (header?.querySelector('.pill')) header.querySelector('.pill').textContent='試作版 v1.20';
  if (phone && header) {
  ...
  ['projectsView','drawingsView'].forEach(id => {
    ...
    if (host) { const label=document.createElement('small'); label.className='sentlog-build'; label.textContent='セントログ v1.20 · PDF再取得・iPhoneページ送り対応'; host.appendChild(label); }
  });
  ...
  const attach = () => {
    ...
    const note=document.createElement('small'); note.className='sentlog-build'; note.textContent='表示バージョン v1.20';
